<?php
/**
 * Profile Controller - DST Recruitment System (CI4)
 */

namespace App\Controllers;

class Profile extends BaseController
{
    /**
     * Lihat Profil
     */
    public function index()
    {
        if ($redirect = $this->requireLogin()) return $redirect;

        $user = db()->row(
            "SELECT * FROM users WHERE user_id = ?",
            [$_SESSION['user_id']]
        );

        $data = [
            'title' => 'Profil Saya - DST Recruitment',
            'page' => 'profile',
            'user' => $user
        ];

        return view('layouts/header', $data) . view('profile/index', $data) . view('layouts/footer');
    }

    /**
     * Edit Profil
     */
    public function edit()
    {
        if ($redirect = $this->requireLogin()) return $redirect;

        $user = db()->row(
            "SELECT * FROM users WHERE user_id = ?",
            [$_SESSION['user_id']]
        );

        $data = [
            'title' => 'Edit Profil - DST Recruitment',
            'page' => 'profile-edit',
            'user' => $user
        ];

        return view('layouts/header', $data) . view('profile/edit', $data) . view('layouts/footer');
    }

    /**
     * Simpan Profil
     */
    public function update()
    {
        if (!isLoggedIn() || $_SERVER['REQUEST_METHOD'] !== 'POST') {
            return redirect()->to(base_url('auth/login'));
        }

        $full_name = sanitize($_POST['full_name'] ?? '');
        $phone = sanitize($_POST['phone'] ?? '');
        $education = sanitize($_POST['education'] ?? '');
        $skills = sanitize($_POST['skills'] ?? '');
        $bio = sanitize($_POST['bio'] ?? '');
        $experience_years = intval($_POST['experience_years'] ?? 0);

        if (empty($full_name)) {
            setFlash('error', 'Nama wajib diisi');
            return redirect()->to(base_url('profile/edit'));
        }

        // Upload avatar jika ada
        $avatarFile = null;
        if (!empty($_FILES['avatar']['name'])) {
            $upload = uploadFile($_FILES['avatar'], 'avatars');
            if ($upload['success']) {
                $avatarFile = $upload['filename'];
            } else {
                setFlash('warning', 'Avatar gagal diupload: ' . $upload['message']);
            }
        }

        $sql = "UPDATE users SET full_name = ?, phone = ?, education = ?, skills = ?, bio = ?, experience_years = ?";
        $params = [$full_name, $phone, $education, $skills, $bio, $experience_years];

        if ($avatarFile !== null) {
            $sql .= ", avatar = ?";
            $params[] = $avatarFile;
        }

        $sql .= " WHERE user_id = ?";
        $params[] = $_SESSION['user_id'];

        db()->execute($sql, $params);

        $_SESSION['full_name'] = $full_name;

        setFlash('success', 'Profil berhasil diperbarui');
        return redirect()->to(base_url('profile'));
    }

    /**
     * Upload foto profil cepat
     */
    public function updateAvatar()
    {
        if (!isLoggedIn() || $_SERVER['REQUEST_METHOD'] !== 'POST') {
            return redirect()->to(base_url('auth/login'));
        }

        if (empty($_FILES['avatar']['name'])) {
            setFlash('warning', 'Pilih file foto terlebih dahulu');
            return redirect()->to(base_url('profile'));
        }

        $upload = uploadFile($_FILES['avatar'], 'avatars');
        if (!$upload['success']) {
            setFlash('error', 'Foto profil gagal diupload: ' . $upload['message']);
            return redirect()->to(base_url('profile'));
        }

        $current = db()->row("SELECT avatar FROM users WHERE user_id = ?", [$_SESSION['user_id']]);

        db()->execute(
            "UPDATE users SET avatar = ? WHERE user_id = ?",
            [$upload['filename'], $_SESSION['user_id']]
        );

        if (!empty($current['avatar']) && $current['avatar'] !== $upload['filename']) {
            $oldAvatarPath = uploadPath('avatars', $current['avatar']);
            if (is_file($oldAvatarPath)) {
                @unlink($oldAvatarPath);
            }
        }

        setFlash('success', 'Foto profil berhasil diperbarui');
        return redirect()->to(base_url('profile'));
    }

    /**
     * Ganti Password
     */
    public function password()
    {
        if ($redirect = $this->requireLogin()) return $redirect;

        $data = [
            'title' => 'Ganti Password - DST Recruitment',
            'page' => 'change-password'
        ];

        return view('layouts/header', $data) . view('profile/password', $data) . view('layouts/footer');
    }

    /**
     * Proses Ganti Password
     */
    public function updatePassword()
    {
        if (!isLoggedIn() || $_SERVER['REQUEST_METHOD'] !== 'POST') {
            return redirect()->to(base_url('auth/login'));
        }

        $current_password = $_POST['current_password'] ?? '';
        $new_password = $_POST['new_password'] ?? '';
        $confirm_password = $_POST['confirm_password'] ?? '';

        // Validasi
        $user = db()->row("SELECT password FROM users WHERE user_id = ?", [$_SESSION['user_id']]);

        if (!verifyPassword($current_password, $user['password'])) {
            setFlash('error', 'Password saat ini salah');
            return redirect()->to(base_url('profile/password'));
        }

        // New password must differ from current password
        if (verifyPassword($new_password, $user['password'])) {
            setFlash('error', 'Password baru harus berbeda dari password saat ini');
            return redirect()->to(base_url('profile/password'));
        }

        if (strlen($new_password) < 6) {
            setFlash('error', 'Password baru minimal 6 karakter');
            return redirect()->to(base_url('profile/password'));
        }

        if ($new_password !== $confirm_password) {
            setFlash('error', 'Password baru tidak cocok');
            return redirect()->to(base_url('profile/password'));
        }

        db()->execute(
            "UPDATE users SET password = ? WHERE user_id = ?",
            [hashPassword($new_password), $_SESSION['user_id']]
        );

        setFlash('success', 'Password berhasil diperbarui');
        return redirect()->to(base_url('profile'));
    }
}
